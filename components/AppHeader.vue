<template>
  <header class="flex justify-center sm:justify-between items-center text-white p-4 font-main sm:mx-6">
    ...
    <div class="hidden lg:block flex-1 text-right space-x-5 font-semibold text-sm">
    ...
    <div class="dark:text-white text-black flex flex-col lg:hidden">
      <button @click="MenuToggle" :aria-label="t('a11y.Menu')">
        <Icon v-if="!MobileMenu" class="h-6 w-6" name="ic:baseline-menu" :aria-label="t('a11y.oMenu')" />
        <Icon v-if="MobileMenu" class="h-6 w-6" name="ic:baseline-close" :aria-label="t('a11y.cMenu')" />
      </button>
      <nav @click.outside="closeMenu" class="dark:(bg-black border-dark-300) bg-light-700 border-1 border-gray-300"
        :class="
          MobileMenu
            ? 'flex p-1 rounded-sm absolute top-12 right-4 sm:right-10 z-1 flex-col-reverse children:pt-2'
            : 'hidden'
        ">
        <NuxtLink v-for="(link, index) in links" :key="index" :to="localePath(link.url)" active-class="text-[#ff0000]"
          :aria-label="t(link.name)"
