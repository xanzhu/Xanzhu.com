<template>
    <header
        class="md:(grid-cols-2 space-y-0 gap-4 m-4 place-items-center) grid grid-cols-1 space-y-5">
        ...
        <div v-if="post.img || post.media" class="relative col-span-1 grow-0">
            <div>
                <NuxtImg v-if="post.media && !post.img" crossorigin="anonymous" class="w-full object-cover border-outline"
                ...
                <GlobalUtilsDate v-if="post.date" :date="post.date"
                    class="dark:(bg-dark-900 text-[#FF0000]) text-[#0066CC] bg-light-500 px-4 py-2 rounded-bl-md md:(rounded-tl-none rounded-br-none rounded-bl-md border-outline font-medium) text-xs absolute top-0 right-0" />
