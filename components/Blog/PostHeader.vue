<template>
    <header class="md:(grid-cols-2 space-y-0 gap-4 m-4 place-items-center) grid grid-cols-1 md:space-y-5">
        <div class="col-span-1 shrink-0 space-y-2 px-4 mt-5 sm:px-0 mb-3 md:(ml-4 mt-0)">
            <div v-if="post.tag"
        ...
        <div v-if="post.img || post.media" class="relative col-span-1 grow-0">
            <div>
                <NuxtImg v-if="post.media && !post.img" crossorigin="anonymous" itemprop="image"
                    class="w-full object-cover md:border-outline" :src="post.media" :alt="post.alt" :title="post.alt"
                ...
                <GlobalUtilsDate v-if="post.date" :date="post.date" itemprop="datePublished"
                    class="dark:(bg-dark-900 text-white) <sm:(border-outline left-0 m-1 text-center) bg-light-500 px-4 py-2 rounded-bl-md md:(rounded-tl-none rounded-br-none rounded-bl-md border-outline font-medium) text-xs absolute top-0 right-0" />
