    ...
    <div
        ...
        <div class="flex flex-col lg:(space-x-0 space-y-3 justify-between flex-row p-12 items-center) space-y-10 p-6">
            ...
            <div class="flex flex-col space-y-5 sm:(flex-row space-x-5 space-y-0)">
                ...
                <button
                    class="border-1 border-gray-300 text-black px-8 py-2 rounded-md text-sm dark:(text-black bg-white) hover:(bg-white) dark:hover:(border-white bg-transparent text-white)">{{ t('links.blog') }}</button>
            </div>
        </div>
        <!-- Wide Feature -->
        <div v-if="wideFeature" v-for="wide in wideFeature" :key="wide.title" class="sm:(mx-20 p-0) p-4">
            <NuxtLink :to="(wide._path)"
                class="text-black dark:(text-white) flex flex-col lg:flex-row lg:items-center mx-auto">
                <!-- Optimise | add alt tags etc -->
                <NuxtImg v-if="wide.img" :src="wide.img" class="dark:(border-dark-300 border-1) rounded-md max-h-xl" />
                <NuxtImg v-else-if="wide.media" :src="wide.media" provider="cloudinary"
                    class="dark:(border-dark-300 border-1) rounded-md max-h-md" />
                ...
                <div class="sm:(space-y-7) md:(p-6) space-y-2">
                    <div v-if="wide.tag"
                        class="dark:(bg-black border-dark-300) border-1 border-gray-300 px-3 py-1 my-2 rounded text-sm inline-flex">
                        {{ wide.tag }}</div>
        ...
        <section class="grid grid-cols-1 gap-10 md:(grid-cols-2) p-4 sm:p-20">
            ...
            <div v-if="features" v-for="(feature, $index) in features" :key="`fe-${$index}`">
                <NuxtLink v-if="feature.feature" class="flex flex-col space-y-3" :to="(feature._path)">
                    <NuxtImg v-if="feature.img" class="dark:(border-dark-300 border-1) rounded-md max-h-xl"
                        :src="feature.img" />
                    <NuxtImg v-else-if="feature.media" :src="feature.media" sizes="sm:100vw md:50vw lg:25vw"
                        provider="cloudinary" class="dark:(border-dark-300 border-1) rounded-md max-h-xl" />
                    <!-- Backup image -->
