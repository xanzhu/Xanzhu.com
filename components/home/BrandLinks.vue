<template>
    <div
        class="mx-2 p1 justify-center space-y-4 mt2 flex flex-col items-center justify-center text-xs font-medium text-center xl:(px5 py2) sm:(flex-row justify-between max-w-[38.5rem] space-y-0 mx-auto) md:(max-w-[46.5rem] px3 py1.5) lg:(max-w-[62.5rem]) xl:(max-w-[72.5rem]) children:(dark:(bg-dark-900 text-white b-dark-700) bg-light-400 text-black px4 py1.5 rounded-sm b-1 b-solid b-gray-300)">
        ...
        <client-only>
            <span class="">{{ formatDate(locale) }}</span>
        </client-only>
    </div>
</template>
